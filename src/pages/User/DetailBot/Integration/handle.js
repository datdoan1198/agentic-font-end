...
export default function Handle() {
    ...
    useEffect(() => {
        if (!_.isEmpty(bot) && _.isEmpty(bot.page)) {
            getListPageFB(bot._id).then((res) => {
    ...
    const facebookLogin = () => {
        ...
        const fbAuthURL = `${import.meta.env.VITE_FB_URL}?client_id=${CLIENT_ID}&redirect_uri=${REDIRECT_URI}&scope=${SCOPES}&response_type=code&state=${bot._id}`;
    ...
    const handleSelectPage = (page_id) => {
        ...
        selectPageFB({page_id}, bot._id).then(() => {
            handleGetDetailBot()
